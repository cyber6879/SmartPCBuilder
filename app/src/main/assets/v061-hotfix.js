...
(function(){
  let dbLimit=160;
  const originalAssistant=window.assistant;

  // Distinguish manufacturer-confirmed, market-catalog-confirmed, and unknown records.
  ...
  // The old sheet looked unchanged when opened without explicit text. Now it always reflects the live assistant message.
  ...
  // Replacement is now actionable instead of only displaying a dead text list.
  ...
  // Database used to silently stop at 120 rows. Now it states coverage and lets the user keep loading everything locally.
  ...
  // Example chips no longer rely on WebView exposing element IDs as global variables.
  window.setNaturalExample=function(text){const el=$('naturalInput');if(!el)return;el.value=text;el.focus();haptic(14);};

  // More informative market coverage summary.
  window.renderCoverage=function(){
    ...
    c.textContent=`本地库 ${DB.length} 条；${DB.filter(x=>x.catalogVerified).length} 条官方目录确认；${DB.filter(x=>x.marketCatalogVerified).length} 条市场目录确认；${priced} 条有可追溯价格，其中 ${high} 条为明确平台快照${conflicts?`；另有 ${conflicts} 条当前价格来源冲突，未强行给单一价格`:''}。`;
  };

  // Show platform/type beside market prices, not just a naked number.
  window.renderMarket=function(){
    ...
    if($('marketList'))$('marketList').innerHTML=(verified.length?verified.map(x=>`<div class="marketItem"><b>${esc(x.name)}</b><div class="price"><strong>${money(x.price)}</strong><small>${esc(x.pricePlatform||'来源可追溯')} · ${esc(x.priceType||'参考价')} · ${esc(x.priceDate||'')}</small></div>${sourceHtml(x)}</div>`).join(''):'<div class="empty">暂无已核验价格。</div>')+conflict.map(x=>`<div class="marketItem"><b>${esc(x.name)}</b><div class="notice warn">当前来源报价冲突：${money(x.priceRange[0])}–${money(x.priceRange[1])}。不选一个数字冒充真实价格。</div>${sourceHtml(x)}</div>`).join('');
  };
  ...
  document.addEventListener('DOMContentLoaded',function(){
    // Compatibility alias for old inline example buttons.
    ...
    else assistant(`v0.6.1 已修复交互并加载 ${DB.length} 条本地记录；当前可追溯价格 ${DB.filter(x=>x.priceVerified).length} 条。`,'good');
  });
})();
